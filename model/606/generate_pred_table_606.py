...
MODEL_PATH = BASE_DIR / "bus_model_606_final.pkl"
FEATURES_PATH = BASE_DIR / "features_606.pkl"
PATTERN_PATH = BASE_DIR / "bus_standard_patterns_jan2026.csv"
OUTPUT_PATH = BASE_DIR / "pred_table_606.csv"
FEATURES_CSV_PATH = BASE_DIR / "features_606.csv"
BUS_MODEL_CSV_PATH = BASE_DIR / "bus_model_606_final.csv"


def main() -> None:
    model = joblib.load(MODEL_PATH)
    features = joblib.load(FEATURES_PATH)
    pattern_df = pd.read_csv(PATTERN_PATH, encoding="utf-8")

    stop_col = pattern_df.columns[0]  # 정류장순번

    # features_606.csv: 피처 이름 목록
    pd.DataFrame({"feature": features}).to_csv(FEATURES_CSV_PATH, index=False, encoding="utf-8-sig")
    print(f"WROTE: {FEATURES_CSV_PATH}")

    # bus_model_606_final.csv: 피처 중요도
    if hasattr(model, "feature_importances_"):
        importance_df = pd.DataFrame(
            {"feature": features, "importance": model.feature_importances_}
        ).sort_values("importance", ascending=False)
        importance_df.to_csv(BUS_MODEL_CSV_PATH, index=False, encoding="utf-8-sig")
        print(f"WROTE: {BUS_MODEL_CSV_PATH}")

    # pred_table_606.csv: 전체 (정류장순번, dow, hour) 조합에 대한 예측값
    ...
    merged = merged.merge(fallback_by_stop_hour, on=[stop_col, "hour"], how="left")
